...
export class OrderNotInternalCodeViewImportPortal {
  ...
  async execute({ search }: { search?: string }) {
    try {
      ...
      and	 p.codigo not in (
        select distinct i.pedidoCod from 01010s005.dev_pedido_v2 p
        inner join 01010s005.dev_pedido_item i on i.nossoNumeroPedido = p.codigo
      )`;
      ...
      const totalItems = Number(
        (
          await dbSiger.$ExecuteQuery<{ total: string }>(
            ...
            inner join 01010s005.dev_pedido_item i on i.pedidoCod = p.codigo
      ...
      console.log(`Pedido 045 sem codigo interno`);
